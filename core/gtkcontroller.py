...
class Controller(xssniperReporter):
    def __init__(self, uifile, mothership, window='window1'):
        wTree = gtk.Builder()
        self.xssniper = xssniper()
    ...
    def on_fly_clicked(self, widget):
        ...
        t = xssniperThread(cmd, self.mothership)
        t.daemon = True
        t.add_reporter(self)
    ...
    def on_targetall_toggled(self, widget):
        """
        Autoconfigure xssniper options to perform an automatic XSS pentesting
        """
        targetall = self.wTree.get_object('targetall')
    ...
    def on_startwizard_clicked(self, widget):
        ...
        commandsenter = self.wTree.get_object('commandsenter')
        commandsenter.set_property("text", "xssniper")
    ...
    def on_reportbug_clicked(self, widget):
        ...
        Report bugs, ideas...
        """
        webbrowser.open("https://github.com/Athexblackhat/xssniper/issues")
    ...
    def generate_command(self):
        ...
        if target_entry.get_active() == False:
            ...
            command.append("--xml")
            command.append("xssniper-test:" + str(datetime.datetime.now()) + ".xml")
        target_entry = self.wTree.get_object('exportpdf')
        if target_entry.get_active() == True:
            command.append("--pdf")
            command.append("xssniper-report:" + str(datetime.datetime.now()) + ".pdf")
        target_entry = self.wTree.get_object('exportjson')
        if target_entry.get_active() == True:
            command.append("--json")
            command.append("xssniper-report:" + str(datetime.datetime.now()) + ".json")
    ...
    def post(self, msg):
        """
        Callback called by xssniper when it has output for the user
        """
        gdk.threads_enter()
        self.post_ui(msg)
        gdk.threads_leave()
...
class xssniperThread(Thread):
    def __init__ (self, cmd, mothership):
        Thread.__init__(self)
        self.app = xssniper(mothership)
        self._cmd = cmd
