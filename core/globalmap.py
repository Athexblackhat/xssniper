...
class DownloadThread(Thread):
    ...
    def run(self):
        ...
        try:
            urllib.request.urlretrieve('/map/GeoLite2-City.dat.gz',
                           geo_db_path+'.gz', reportfunc)
        except:
            try:
                urllib.request.urlretrieve('https://xssniper.sf.net/map/GeoLite2-City.dat.gz',
                           geo_db_path+'.gz', reportfunc)
            except:
                self._parent.report_state('error downloading map', 0.0)
                self._map.geomap_failed()
        else:
            self._parent.report_state('map downloaded (restart xssniper!!!!)', 0.0)
...
class GlobalMap(gtk.DrawingArea, xssniperReporter):
    ...
    def get_geodb_path(self):
        ...
        if os.path.exists(os.path.join(gtkpath, 'map/GeoIP.dat')):
            ...
            home = str(Path.home())
            return os.path.join(home, '.xssniper', 'GeoIP.dat')
