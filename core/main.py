...
from core.options import xssniperOptions
...
from core.reporter import xssniperReporter
...
class xssniper(EncoderDecoder, xssniperReporter):
    """
    xssniper application class
    """
    def __init__(self, mothership=None):
        ...
        # xssniper verbosity (0 - no output, 1 - dots only, 2+ - real verbosity)
        self.verbose = 2
        self.options = None
    ...
    def get_gtk_directory(self):
        ...
        if os.path.exists(local_path):
            ...
            return self._gtkdir
        elif os.path.exists('/usr/share/xssniper/gtk'):
            self._gtkdir = '/usr/share/xssniper/gtk'
            return self._gtkdir
    ...
    def not_keyword_exit(self):
        self.report("="*30)
        self.report("\n[Error] xssniper cannot find a correct place to start an attack. Aborting!...\n")
        self.report("-"*25)
        self.report("\n[Info] This is because you aren't providing:\n\n At least one -payloader- using a keyword: 'XSS' (for hex.hash) or 'X1S' (for int.hash):\n")
        self.report("  - ex (GET): xssniper -u 'https://target.com' -g '/path/profile.php?username=bob&surname=XSS&age=X1S&job=XSS'")
        self.report("  - ex (POST): xssniper -u 'https://target.com/login.php' -p 'username=bob&password=XSS&captcha=X1S'\n")
        self.report(" Any extra attack(s) (Xsa, Xsr, Coo, Dorker, Crawler...):\n")
        self.report("  - ex (GET+Cookie): xssniper -u 'https://target.com' -g '/path/id.php?=2' --Coo")
        self.report("  - ex (POST+XSA+XSR+Cookie): xssniper -u 'https://target.com/login.php' -p 'username=admin&password=admin' --Xsa --Xsr --Coo")
        self.report("  - ex (Dorker): xssniper -d 'news.php?id=' --Da")
        self.report("  - ex (Crawler): xssniper -u 'https://target.com' -c 100 --Cl\n")
        self.report(" Or a mixture:\n")
        self.report("  - ex (GET+Manual): xssniper -u 'https://target.com' -g '/users/profile.php?user=XSS&salary=X1S' --payload='<script>alert(XSS);</script>'")
        self.report("  - ex (POST+Manual): xssniper -u 'https://target.com/login.asp' -p 'username=bob&password=XSS' --payload='}}%%&//<sc&ri/pt>(XSS)--;>'\n")
        self.report("  - ex (GET+Cookie): xssniper -u 'https://target.com' -g '/login.asp?user=bob&password=XSS' --Coo")
        self.report("  - ex (POST+XSR+XSA): xssniper -u 'https://target.com/login.asp' -p 'username=bob&password=XSS' --Xsr --Xsa\n")
        self.report("="*75 + "\n")
        if not self.options.xssniper_gtk and not self.mothership:
            sys.exit(2)
        else:
            self._landing = True
    ...
    def _report_attack_failure(self, curl_handle, dest_url, payload,\
        ...
        if str(curl_handle.info()["http-code"]) == "404":
            ...
            self.report("\n[Error] 407 Proxy Authentication Required: xssniper must first authenticate itself with the proxy\n")
        elif str(curl_handle.info()["http-code"]) == "408":
            self.report("\n[Error] 408 Request Timeout: xssniper did not produce a request within the time that the server was prepared to wait\n")
            ...
            self.report("\n[Error] xssniper (or your TARGET) is not working properly...\n\n - Wrong URL\n - Firewall\n - Proxy\n - Target offline\n - [?] ...\n")
        else:
    ...
    def create_options(self, args=None):
        ...
        """
        self.optionParser = xssniperOptions()
    ...
    def _get_attack_urls(self):
        ...
        if options.update:
            ...
            try:
                ...
                print("$ git clone https://github.com/Athexblackhat/xssniper\n")
           ...
           if self.user_template is not None:
               ...
               if len(self.user_template) > 6:
                   if self.user_template[6] == "PDF":
                       self.options.filepdf = "xssniper_report.pdf"
                   elif self.user_template[6] == "XML":
                       self.options.filexml = "xssniper_report.xml"
        ...
        if options.target: # miau!
            ...
            self.options.filexml = "xssniper_" + str(self.options.target) + "_" + str(datetime.datetime.now())+".xml" # export xml
        ...
        if not options.imx or not options.xssniper_gtk or not options.update:
            return urls
    ...
    def xssniper_reports_dir(self, filename):
    ...
    def get_command_used(self):
        return "xssniper " + " ".join(sys.argv[1:])
    ...
    def print_results(self):
        ...
        if self.options.filexml:
            self.options.filexml = self.xssniper_reports_dir(self.options.filexml)
        if self.options.filepdf:
            self.options.filepdf = self.xssniper_reports_dir(self.options.filepdf)
        if self.options.filejson:
            self.options.filejson = self.xssniper_reports_dir(self.options.filejson)
        self.report('='*75)
        ...
        if self.options.fileoutput:
            fout = open(self.xssniper_reports_dir("XSSreport.raw"), "w") # write better than append
            fout.write("="*75 + "\n")
            fout.write("xssniper Security Report: " + str(datetime.datetime.now()) + "\n")
            fout.write("="*75 + "\n\n")
        ...
        for line in self.hash_found:
            ...
            if line[2] == "XSR":
                ...
                if self.options.fileoutput:
                    fout.write("="*75)
                    fout.write("\n" + "xssniper Security Report: " + str(datetime.datetime.now()) + "\n")
                    fout.write("="*75 + "\n\n")
        ...
        if self.options.statistics:
	        ...
	        # algor= vulnerables + false positives - failed * extras
                ...
                if total_payloads > 100:
                    mana = mana + 150
                if not self.options.xssniper_gtk:
                    mana = mana + 25
                if self.options.discode:
                    mana = mana + 100
        ...
        if not len(self.hash_found) and self.hash_notfound:
            ...
            if len(self.hash_found) + len(self.hash_notfound) == 0 and not Exception:
                self.report("\n[Error] xssniper cannot send any data... maybe -something- is blocking connection(s)!?\n")
            if len(self.hash_found) + len(self.hash_notfound) == 0 and self.options.crawling:
                if self.options.xssniper_gtk or self.options.target:
                    self.report('='*75)
