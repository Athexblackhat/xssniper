...
class xssniperOptions(optparse.OptionParser):
    def __init__(self, *args):
        ...
        self.add_option("--gtk", action="store_true", dest="xssniper_gtk", help="launch xssniper GTK Interface")
        ...
        group5.add_option("--reverse-check", action="store_true", dest="reversecheck", help="Establish a reverse connection from target to xssniper")
        ...
        group6.add_option("--auto", action="store_true", dest="fuzz", help="AUTO  - Inject a list of vectors provided by xssniper")
        self.add_option_group(group6)

        group14 = optparse.OptionGroup(self, "*Select Payload(s)*",
        "These options can be used to set the list of vectors provided by xssniper. Choose only if required:")
    ...
    def get_options(self, user_args=None):
        ...
        if (not options.url and not options.readfile and not options.dork and not options.dork_file and not options.imx and not options.update and not options.xssniper_gtk and not options.wizard and not options.xst and not options.target and not options.checktor and not options.reqfile and not sys.stdin.isatty()):
            piped = sys.stdin.read().strip() # stdin/pipe mode: cat urls.txt | xssniper
            if piped:
                tmp = tempfile.NamedTemporaryFile(mode='w', prefix='xssniper_stdin_', suffix='.txt', delete=False)
                tmp.write(piped)
                tmp.close()
                options.readfile = tmp.name
        if (not options.url and not options.readfile and not options.dork and not options.dork_file and not options.imx and not options.update and not options.xssniper_gtk and not options.wizard and not options.xst and not options.target and not options.checktor and not options.reqfile):
            print("\n"+ '='*75)
            print(self.version)
            print("-----------", "\n")
            ...
            print("                              || == < ==   * * \\//))//)  BBzzzzz!               ")
            print("Forum:                        ||]~~/ \\~~[ *    (())//))                         ")
            print("irc.libera.chat -> #xssniper     ||   (')          \\/())/                          ")
            print("                              ||  /  /            \\/                            ")
            print("")
            print('='*75)
