...
class TestOptions(unittest.TestCase):
    def setUp(self):
        ...
        self.o = xssniperOptions()
    def test_version(self):
        self.assertIn('1.0', self.o.version)
        self.assertIn('ATHEX', self.o.version)
...
class TestReverseCheck(unittest.TestCase):
    def test_option(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--reverse-check", "-u", "http://127.0.0.1"])
        self.assertTrue(opts.reversecheck)
    def test_hub_machinery(self):
        ...
        self.assertTrue(xssniperReporter)
...
class TestIntegration(unittest.TestCase):
    def test_end_to_end_reflected_xss(self):
        ...
        t.start()
        try:
            r = subprocess.run([sys.executable, "xssniper", "-u", "http://127.0.0.1:%d/?q=XSS" % port,
...
class TestCookieInjection(unittest.TestCase):
    def test_coo_header_injection(self):
        ...
        try:
            r = subprocess.run([sys.executable, "xssniper", "-u", "http://127.0.0.1:%d/" % port,
...
def _run(args, timeout=60):
    r = subprocess.run([sys.executable, "xssniper"] + args + ["--debug"], cwd=ROOT,
...
class TestNewIO(unittest.TestCase):
    ...
    def test_stdin_pipe_mode(self):
        base = "http://127.0.0.1:%d/?q=XSS" % _MOCK["port"]
        r = subprocess.run([sys.executable, "xssniper", "--auto", "--auto-set", "2", "--timeout", "5", "--debug"],
...
class TestAuth(unittest.TestCase):
    ...
    def test_auth_cert_parses(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--auth-cert", "key.pem,cert.pem", "-u", "http://127.0.0.1"])
...
class TestCheckTor(unittest.TestCase):
    ...
    def test_check_tor_proxy_parses(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--check-tor", "--proxy", "http://127.0.0.1:8118"])
...
class TestCemEncoders(unittest.TestCase):
    ...
    def test_cem_parses(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--Cem", "Mix,Une,Str,Hex", "-u", "http://127.0.0.1"])
...
class TestMiscellaneous(unittest.TestCase):
    ...
    def test_silent_mode(self):
        base = "http://127.0.0.1:%d/?q=XSS" % _MOCK["port"]
        r = subprocess.run([sys.executable, "xssniper", "-u", base, "--auto", "--auto-set", "1",
                            "--silent", "--timeout", "5"], cwd=ROOT, capture_output=True, text=True, timeout=60)
        out = r.stdout + r.stderr
        self.assertNotIn("Traceback", out)
    def test_alive_parses(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--alive", "3", "-u", "http://127.0.0.1"])
        self.assertEqual(opts.isalive, 3)
    def test_update_parses(self):
        from core.options import xssniperOptions
        opts = xssniperOptions().get_options(["--update"])
